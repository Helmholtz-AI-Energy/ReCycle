...
class MultiLayerPerceptron(Model):
    def __init__(self, model_spec: MLPSpec) -> None:
        ...
        if model_spec.hidden_layers is not None:
            ...
            if model_spec.nr_of_hidden_layers == 0:
                layers.append(nn.Linear(self.input_features, self._output_features, device=self.device))
            else:
                layers.extend([
                    nn.Linear(self.input_features, self.d_model, device=self.device),
                    nn.Dropout(p=model_spec.dropout),
                    model_spec.non_linearity()
                ])
                ...
                layers.append(nn.Linear(self.d_model, self._output_features, device=self.device))
    ...
    def forward(self, input_sequence: Tensor, batch_size: int, input_metadata: Optional[Tensor] = None,
        ...
        return output_sequence.unflatten(-1, (self.forecast_window, self.output_features))
