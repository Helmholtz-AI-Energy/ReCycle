...
class MAPELoss(ErrorMetric):
    @staticmethod
    def forward(
        prediction: Tensor, reference: Tensor, time_resolved: bool = False
    ) -> Tensor:
        ...
        if time_resolved:
            dims = tuple(range(normalized.dim()))[
                :-2
            ]  # last two are assumed to be time dimensions and maintained
...
class Bias(ErrorMetric):
    ...
    def forward(
        ...
        if time_resolved:
            dims = tuple(range(deviation.dim()))[
                :-2
            ]  # last two are assumed to be time dimensions and maintained
...
class RPELoss(ErrorMetric):
    ...
    def forward(prediction: Tensor, reference: Tensor) -> Tensor:
        ...
        prediction_peak = torch.gather(
            prediction.flatten(start_dim=-2), dim=0, index=position.unsqueeze(dim=-1)
        )
...
class PAPELoss(ErrorMetric):
    ...
    def forward(prediction: Tensor, reference: Tensor) -> Tensor:
        ...
        normalized = torch.abs(1 - torch.div(prediction, reference + eps))
        result = torch.mean(
            100 * torch.max(normalized.flatten(start_dim=-2), dim=-1)[0]
        )
        return result
...
time_resolvable_metrics = dict(mape=MAPELoss, mae=MAELoss, mse=MSELoss, bias=Bias)


def apply_error_metric(
    prediction: Tensor, reference: Tensor, metric: str or list = "all"
) -> pd.DataFrame:
    if metric == "all":
