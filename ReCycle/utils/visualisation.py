...
def plot_sample(
    ...
    if (historic_time is None) != (forecast_time is None):
        raise ValueError(
            f'Either both or neither of historic_time ({"None" if historic_time is None else "not None"}) '
    ...
    if historic_time is None:
        ...
        forecast_time = torch.arange(
            len(historic_data), len(forecast_data) + len(historic_data)
        )
...
def plot_losses(
    train_loss: Tensor, valid_loss: Tensor, truncate: int = 0
) -> (plt.Figure, plt.Axes):
    fig, ax = plt.subplots()
...
def plot_calibration(
    predicted: Tensor, target: Tensor, dpi: Optional[int] = 600
) -> (plt.Figure, plt.Axes):
    plt.rc("font", size=4)
