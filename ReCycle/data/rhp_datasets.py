...
class LooseTypeRHPDataset(Dataset):
    def __init__(
        ...
        grouped_data.loc[:, "date"] = pd.to_datetime(
            input_data.loc[:, time_column]
        ).dt.date
        ...
        assert (
            grouped_data.size().values == group_len
        ).all(), "Inconsistent day lengths"
        ...
        for country in target:
            ...
            for group in grouped_data:
                ...
                reference_days[day_type] = self.update_ref_day(
                    reference_days[day_type], torch.tensor(data[data_column].values)
                )

                if len(reference_days[day_type]) != 0:
                    last_rhp[day_type] = torch.nanmean(
                        torch.stack(reference_days[day_type], dim=0), dim=0
                    )
    ...
    def get_local_rhp(self) -> Tensor:
        day_selector_matrix = self.day_types[:, :, None, None].expand(
            -1, -1, -1, self.cycle_len
        )
        local_rhp = torch.gather(self.rhp, 2, day_selector_matrix).squeeze(-2)
        return local_rhp

    def get_rhp(
        self, country_idx: int, historic_slice: slice, forecast_slice: slice
    ) -> Tuple[Tensor, Tensor]:
        ...
        day_selector_matrix = historic_day_types[:, None, None].expand(
            -1, -1, self.cycle_len
        )
