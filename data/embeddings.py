...
def sine_positional_encoding(max_pos: int, d_model: int, freq: float = 1e-4) -> Tensor:
    ...
    freqs = freq**(2*torch.div(torch.arange(d_model), 2, rounding_mode='floor')/d_model)
...
class BasicEmbedding(nn.Module):
    ...
    def __init__(self, input_features: int, output_features: int, device: torch.device = None):
...
class VoidEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int = 0, device: torch.device = None):
...
class IdentityEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int = None, device: torch.device = None):
...
class ConvolutionalEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int, device: torch.device = None,
                 kernel_size: int = 3, padding: int = 1, padding_mode: str = 'circular', **kwargs):
        super(ConvolutionalEmbedding, self).__init__(input_features, output_features, device)

        self.conv = nn.Conv1d(in_channels=input_features, out_channels=output_features, kernel_size=kernel_size,
                              padding=padding, padding_mode=padding_mode, device=device, **kwargs)
...
class LinearAugmentation(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int = None, device: torch.device = None,
        ...
        assert output_features == (input_features + 1), \
            f'For linear augmentation {input_features=}+1 must equal {output_features=}'

        super(LinearAugmentation, self).__init__(input_features, output_features, device)
        pe = torch.true_divide(torch.arange(max_len), max_len-1).unsqueeze(0).unsqueeze(-1)
        self.register_buffer('pe', pe)  # TODO: Why save this with state dict?

    def forward(self, input_sequence: Tensor) -> Tensor:
        pe = self.pe[:, :input_sequence.shape[1], :].expand(input_sequence.shape[0], -1, -1)
        output = torch.cat((input_sequence, pe), dim=2)
        return output


class BasePositionalEncoding(BasicEmbedding):
    def __int__(self, input_features: int, output_features: int = None, device: torch.device = None,
                max_pos: int = 200, freq: float = 1e-4):
        output_features = output_features or input_features
        assert output_features == input_features, 'Output features mut equal input features'
        super(BasePositionalEncoding, self).__int__(input_features, output_features, device)
    ...
    def forward(self, input_sequence: Tensor) -> Tensor:
        ...
        if input_sequence.dim() == 3:
            ...
            positional_code = self.positional_encoding[:seq_len].expand(batch_size, -1, -1)
            return positional_code
        elif input_sequence.dim() == 2:
            positional_code = self.positional_encoding[:seq_len]
            return positional_code
        else:
            raise TypeError(f'Invalid input sequence shape!'
                            f'Input sequence dimension {input_sequence.dim()} should be 2 or 3')


class PeriodicAugmentation(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int = None, device: torch.device = None,
                 period: int = 24, offset: int = 0, max_len: int = 100):
        output_features = output_features or input_features + 1
        assert output_features == (input_features + 1), f'For periodic augmentation {input_features=}+1 must equal {output_features=}'
        super(PeriodicAugmentation, self).__init__(input_features, output_features, device)
        pe_period = torch.true_divide(torch.arange(period), period).roll(offset, 0)
        period_nr = max_len // period + 1  # poor man's round up division to see how many periods cover max_length
...
class InformerPositionalEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int, device: torch.device = None):
        super(InformerPositionalEmbedding, self).__init__(input_features, output_features, device)

        max_length = 5000
        dtype = torch.float
        pe = torch.zeros((max_length, output_features), requires_grad=False, device=self.device)
        position = torch.arange(max_length, dtype=dtype).unsqueeze(dim=1)
        # Compute positional encoding in log space
        div_term = torch.exp(torch.arange(0, output_features, step=2, dtype=dtype)
                             * -(log(2 * max_length)) / output_features)
...
class InformerConvolutionalEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int, device: torch.device = None):
        super(InformerConvolutionalEmbedding, self).__init__(input_features, output_features, device)

        self.conv = nn.Conv1d(in_channels=input_features, out_channels=output_features, kernel_size=3,
                              padding=1, padding_mode='circular', device=device)
...
class InformerFixedEmbedding(BasicEmbedding):
    def __init__(self, input_features: int, output_features: int, device: torch.device = None):
        super(InformerFixedEmbedding, self).__init__(input_features, output_features, device)
        max_length = 5000
        dtype = torch.float
        pe = torch.zeros((max_length, output_features), requires_grad=False, dtype=dtype, device=self.device)
...
class FullEmbedding(nn.Module):
    ...
    def __init__(self, data_embedding: BasicEmbedding, metadata_embedding: BasicEmbedding, output_features: int):
        assert data_embedding.device == metadata_embedding.device, f'Both embeddings must be on same device'
        super(FullEmbedding, self).__init__()

        input_features = (data_embedding.input_features, metadata_embedding.input_features)
    ...
    def forward(self, input_sequence: Tensor, metadata: Optional[Tensor] = None) -> Tensor:
...
class AdditiveFullEmbedding(FullEmbedding):
    def __init__(self, data_embedding: BasicEmbedding, metadata_embedding: BasicEmbedding):
        if metadata_embedding.output_features != 0:
            assert data_embedding.output_features == metadata_embedding.output_features, \
        ...
        super(AdditiveFullEmbedding, self).__init__(data_embedding, metadata_embedding, output_features)
...
class ConcatFullEmbedding(FullEmbedding):
    def __init__(self, data_embedding: BasicEmbedding, metadata_embedding: BasicEmbedding):
        output_features = data_embedding.output_features + metadata_embedding.output_features

        super(ConcatFullEmbedding, self).__init__(data_embedding, metadata_embedding, output_features)
...
class InputLinearAugmentation(ConcatFullEmbedding):
    def __init__(self, input_features: int, output_features: int = None, device: torch.device = None,
                 max_len: int = 100):
        data_embedding = LinearAugmentation(input_features, output_features, device, max_len)
        metadata_embedding = VoidEmbedding(input_features, device=device)

        super(InputLinearAugmentation, self).__init__(data_embedding, metadata_embedding)


class MetadataAugmentation(ConcatFullEmbedding):
    # Straight concat of data and metadata_dict
    def __init__(self, input_features: int, metadata_features: int, device: torch.device = None):
